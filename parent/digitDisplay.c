...
int DigitDisplay_displayNum(int num)
{
    ...
    digiDispWrite(displayCharState[AlarmMonitor_getDispMode()]);
...
static void digiDispTask(void)
{
	while(!AlarmMonitor_isStopping()){
		dispMode_t dispMode = AlarmMonitor_getDispMode();

		if(dispMode == dispModeAlarmArm){
			if(AlarmMonitor_getAlarmArm())
				//Display '1111' if armed
				DigitDisplay_displayNum(1111);
			else
				//Display '0000' if not armed
				DigitDisplay_displayNum(0);

		}else if(dispMode == dispModeAlarmSound)
			//Display current buzzer mode number
			DigitDisplay_displayNum(AlarmMonitor_getAlarmBuzzMode());

		else if(dispMode == dispModeTemp)
			DigitDisplay_displayNum(AlarmMonitor_getBbyRoomTemp());

		else if(dispMode == dispModeSound)
			DigitDisplay_displayNum(AlarmMonitor_getBbySoundLevel());
